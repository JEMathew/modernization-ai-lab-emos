...
function enterpriseContextStatus(id, snapshot) {
  ...
  if (id === "program") {
    const programSummary = programIntelligence?.programSummary();
    if (programSummary?.propagation.status === "Complete") return "PROTECTED BOUNDARY PROPAGATED";
    if (programSummary?.constraint.approvalStatus === "Approved") return "PROGRAM DECISION APPROVED";
    if (programSummary?.constraint.approvalStatus === "Evidence Requested") return "PROGRAM EVIDENCE REQUESTED";
    if (programSummary?.constraint.approvalStatus === "Rejected") return "PROGRAM CONSTRAINT REJECTED";
...
function renderEnterpriseContext() {
  ...
  $$('[data-enterprise-context-detail]').forEach((detail) => {
    ...
    const programDecision = node.id === "program" ? programIntelligence.programSummary() : null;
    detail.innerHTML = `<div class="enterprise-context-detail-heading"><div><small>${node.type.toUpperCase()} / ${node.reference}</small><h3>${node.name}</h3></div><strong>${enterpriseContextStatus(node.id, snapshot)}</strong></div><div class="enterprise-context-detail-grid"><span><small>ACCOUNTABLE OWNER</small><strong>${owner}</strong></span><span><small>PURPOSE</small><strong>${node.purpose}</strong></span><span><small>EXPECTED BUSINESS OUTCOME</small><strong>${node.outcome}</strong></span><span><small>RELATIONSHIP</small><strong>${node.relationship}</strong></span><span><small>NEXT RESPONSIBILITY</small><strong>${programDecision ? programDecision.blocker : nextResponsibility}</strong></span>${programDecision ? `<span><small>GOVERNED PROGRAM CONSTRAINT</small><strong>${programDecision.constraint.id} · ${programDecision.constraint.approvalStatus} · ${programDecision.constraint.dependency}</strong></span>` : ""}</div>`;
  });
}
...
function renderProgramIntelligence() {
  ...
  const programObjects = programIntelligence.programWorkObjects();
  const decisionLocked = summary.propagation.status === "Complete";
  $$('[data-program-intelligence]').forEach((panel) => {
    panel.innerHTML = `<header><div><small>V1.3 / GOVERNED PROGRAM COORDINATION</small><strong>${summary.name}</strong></div><span>${summary.caseCount} GOVERNED CASES</span></header>
      ...
      <section class="program-decision" data-program-decision-state="${constraint.approvalStatus.toLowerCase().replaceAll(" ", "-")}">
        <header><div><small>PROGRAM CONSTRAINT / ${constraint.id}</small><h3>Protect the Supplier Master API interface</h3></div><strong>${constraint.approvalStatus.toUpperCase()}</strong></header>
        <div class="constraint-facts"><span><small>DESCRIPTION</small><strong>${constraint.description}</strong></span><span><small>BUSINESS RATIONALE</small><strong>${constraint.businessRationale}</strong></span><span><small>DEPENDENCY</small><strong>${constraint.dependency}</strong></span><span><small>OWNER / DURATION</small><strong>${constraint.owner} · ${constraint.proposedDuration}</strong></span><span><small>AFFECTED CASES</small><strong>${constraint.affectedCases.join(" · ")}</strong></span><span><small>DOWNSTREAM OBLIGATIONS</small><strong>${constraint.downstreamObligations.join(" · ")}</strong></span></div>
        <div class="program-impact-grid">${impacts.map((impact) => `<button type="button" data-program-action="inspect-impact" data-case-id="${impact.caseId}"><small>${impact.impactType.toUpperCase()} IMPACT / ${impact.caseId}</small><strong>${programIntelligence.cases[impact.caseId].name}</strong><span>${impact.proposedChanges.join(" · ")}</span><em>${impact.unaffectedWork}</em><b>${impact.status || "PROPOSED"}</b></button>`).join("")}</div>
        <div class="program-decision-evidence"><div><small>CONTEXTUAL EVIDENCE</small><strong>${constraint.evidence.join(" · ")}</strong></div><div><small>DECISION REQUIRED</small><strong>Should the Mission Commander approve a six-month Protected Interface Boundary?</strong></div></div>
        <div class="program-decision-actions" role="group" aria-label="Mission Commander program decision"><button type="button" data-program-action="program-decision" data-decision-option="approve" ${decisionLocked ? "disabled" : ""}>Approve Protected Interface Boundary</button><button type="button" data-program-action="program-decision" data-decision-option="evidence" ${decisionLocked ? "disabled" : ""}>Request More Evidence</button><button type="button" data-program-action="program-decision" data-decision-option="reject" ${decisionLocked ? "disabled" : ""}>Reject Program Constraint</button>${constraint.approvalStatus === "Approved" && summary.propagation.status !== "Complete" ? `<button class="is-primary" type="button" data-program-action="propagate-program">Propagate Approved Constraint</button>` : ""}<button type="button" data-program-action="reset-program">Reset Program Decision</button></div>
        ${decision ? `<div class="program-decision-record"><small>PROGRAM DECISION RECORD / ${decision.id}</small><strong>${decision.selectedOption}</strong><span>${decision.approver} · ${decision.sequenceMarker} · PROPAGATION ${decision.downstreamPropagationStatus.toUpperCase()}</span><p>${decision.rationale}</p></div>` : `<div class="program-decision-record is-pending"><small>PROGRAM DECISION RECORD</small><strong>NOT YET RECORDED</strong><span>The constraint is visible but has not been applied.</span></div>`}
        <div class="program-object-grid">${programObjects.map((object) => `<button type="button" data-program-action="inspect-program-object" data-object-id="${object.id}" ${object.status === "Locked" ? "disabled" : ""}><small>${object.id} · ${object.status.toUpperCase()}</small><strong>${object.title}</strong><span>${object.caseId || object.programId}</span></button>`).join("")}</div>
      </section>
      <footer><strong>PROGRAM SEQUENCE</strong><span>${summary.sequence.join(" → ")}</span><p>${summary.rationale}</p><div><small>SHARED DEPENDENCY · REPRESENTED ONCE</small><strong>${summary.sharedDependencies[0].name}</strong><span>Direct: DR-SQP-002 · Indirect: DR-CIC-001 · ${summary.sharedDependencies[0].status} · ${summary.affectedCaseCount} propagated impacts</span></div></footer>`;
  });
}
...
function handleProgramAction(button) {
  ...
  if (action === "propagate-program") { programIntelligence.propagateProgramConstraint(); renderHqState(); }
  if (action === "reset-program") { programIntelligence.resetProgram(); renderHqState(); }
